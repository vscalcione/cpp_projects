...
main()
{
	...
	{
		do //secondo ciclo do-while che permette di ripetere il menù (elenco) delle possibili operazioni da poter eseguire
		{
		...
		while (scelta<1 or scelta>8); //il menù verrà ripetuto fino a quando verrà inserito un valore minore di 1 o maggiore di 8
		if (scelta==1)
		{
			system ("cls");
...
{
	if (b==0 or a==0) //se il primo/secondo valore inserito è uguale a 0, verrà stampato il valore 0
	{
		return 0;
	}
	else
	{
		int q;
		q=a/b; //nel caso contrario, ovvero che il primo/secondo valore inserito è diverso da 0, viene eseguito correttamente il calcolo del
		       //nostro quoziente
		return q; 
	}
}
...
int fattoriale (int numero) //definizione funzione calcolo fattoriale
{
	...
	if (numero==0)
	{
		return 1; //se il numero inserito è uguale a 0, verrà  stampato 11
	}
	else
	{
		j=numero*fattoriale (numero-1); //se invece, il nostro numero inserito è maggiore di 0, verà calcolato il fattoriale di tale numero
...
int potenza (int base, int esponente) //definizione funzione calcolo potenza
{
	...
	if (esponente==0)
	{
		return 1; //nel caso in cui l'esponente inserito è uguale a 0, si stamperà 1
	}
	else
	{
		h=base*potenza (base,esponente-1); //mentre invece, nel caso in cui l'esponente inserito è maggiore di 0, verrà calcolata la nostra
		                                  //potenza
	}
	return h; 
}
